...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
		...
		self.mapImageView = [[UIImageView alloc] initWithFrame:CGRectMake(5, 5, 54, 54)];
		[self.mapImageView setBackgroundColor:[UIColor whiteColor]];
		[self.mapImageView setClipsToBounds:YES];
		[self.mapImageView setContentMode:UIViewContentModeScaleAspectFill];
		[self.mapImageView.layer setBorderColor:[UIColor colorWithWhite:0 alpha:0.3].CGColor];
		[self.mapImageView.layer setBorderWidth:1];
        [self.mapImageView.layer setCornerRadius:2];
...
-(UIImageView*)checkmarkMapImageViewView {
    if (!_checkmarkMapImageView) {
        _checkmarkMapImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"CheckmarkProfileImage"]];
        [_checkmarkMapImageView setFrame:CGRectMake(CGRectGetMaxX(self.mapImageView.frame) - 10 - CGRectGetWidth(_checkmarkMapImageView.frame), CGRectGetMaxY(self.mapImageView.frame) - 10 - CGRectGetHeight(_checkmarkMapImageView.frame), CGRectGetWidth(_checkmarkMapImageView.frame), CGRectGetHeight(_checkmarkMapImageView.frame))];
    }
    return _checkmarkMapImageView;
}
...
-(void)prepareForReuse {
    ...
    [_checkmarkMapImageView removeFromSuperview];
    _checkmarkMapImageView = nil;
    [self cleanupBackView];
}

-(void)layoutSubviews {
    [super layoutSubviews];
    self.textLabel.frame = CGRectMake(CGRectGetMaxX(self.mapImageView.frame) + 10, CGRectGetMinY(self.textLabel.frame), CGRectGetWidth(self.textLabel.frame), CGRectGetHeight(self.textLabel.frame));
    self.detailTextLabel.frame = CGRectMake(CGRectGetMaxX(self.mapImageView.frame) + 10, CGRectGetMinY(self.detailTextLabel.frame), CGRectGetWidth(self.detailTextLabel.frame), CGRectGetHeight(self.detailTextLabel.frame));
}
...
-(void)setFavourite:(BOOL)favourite animated:(BOOL)animated {
    ...
    if (animated) {
        if (favourite) {
            ...
            [self.mapImageView.layer addAnimation:animation forKey:@"animateBorderColor"];
            [self.mapImageView.layer setBorderColor:[UIColor colorWithRed:0.149 green:0.784 blue:0.424 alpha:0.750].CGColor];
            [self.checkmarkMapImageViewView setAlpha:0];
            [self.mapImageView addSubview:_checkmarkMapImageView];
            [UIView animateWithDuration:0.25
                             animations:^{
                                 [self.checkmarkMapImageViewView setAlpha:1];
                             }];
        } else {
            CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"borderColor"];
